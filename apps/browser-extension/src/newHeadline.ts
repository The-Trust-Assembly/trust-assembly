...
async function initDefaults(): Promise<void> {
  ...
  var HeadLineString = await browser.storage.local.get(["new Headline:"+currentUrl]);
  ...
  else
  {
    ...
    active: true, currentWindow: true}).then((tabs) => {
      ...
      tab.id, 
      {action: "getHeadlineText"}).then((response) => {
        if (browser.runtime.lastError) {
          console.error('Message failed:', browser.runtime.lastError);
        }
...
let SaveHandler = async (event: MouseEvent) => {
  try {
    currentUrl = await getCurrentUrl();
    browser.storage.local.set({ ["new Headline:"+currentUrl]: JSON.stringify({
