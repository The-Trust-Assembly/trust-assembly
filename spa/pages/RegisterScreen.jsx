...
export default function RegisterScreen({ onRegister }) {
  ...
  return (
    <div style={{ maxWidth: 500, margin: "0 auto", padding: "0 16px", "--bg": "#f5f2ec", "--card-bg": "#ffffff", "--border": "#d9d3c7", "--gold": "#b8922e", "--text": "#1a1714", "--text-sec": "#5c564d", "--text-muted": "#9a948b", "--green": "#2d7a38", "--red": "#b03a2e", color: "#1a1714" }}>
      ...
      <div style={{ padding: 10, background: "rgba(212,168,67,0.09)", border: "1.5px solid #B45309", borderRadius: 0, marginBottom: 14, fontSize: 12, color: "var(--text-sec)", lineHeight: 1.6 }}>
        <strong style={{ color: "var(--gold)" }}>BETA:</strong> This is an experimental platform under active development. Do not enter sensitive personal information. Use a pseudonym if you prefer. Data may be reset.
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div className="ta-field"><label>Gender *</label><select value={form.gender} onChange={e => { s("gender", e.target.value); if (e.target.value === "di") s("isDI", true); else { s("isDI", false); s("diPartner", ""); } }} style={{ width: "100%", padding: "10px 8px", border: "1px solid var(--border)", background: "var(--card-bg)", fontSize: 13, borderRadius: 0, color: form.gender ? "var(--text)" : "var(--text-muted)" }}><option value="">Select</option><option value="male">Male</option><option value="female">Female</option><option value="nonbinary">Non-binary</option><option value="other">Other</option><option value="undisclosed">Prefer not to say</option><option value="di">N/A, I am an AI Agent</option></select></div>
      ...
      {!form.isDI && <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div className="ta-field"><label>Country *</label><select value={form.country} onChange={e => { s("country", e.target.value); s("region", ""); s("politicalAffiliation", ""); }} style={{ width: "100%", padding: "10px 8px", border: "1px solid var(--border)", background: "var(--card-bg)", fontSize: 13, borderRadius: 0, color: form.country ? "var(--text)" : "var(--text-muted)" }}><option value="">Select country</option>{COUNTRIES.map((c, i) => c.disabled ? <option key={`sep-${i}`} disabled>──────────</option> : <option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
        {form.country && STATES_BY_COUNTRY[form.country] && <div className="ta-field"><label>{form.country === "Canada" ? "Province / Territory" : form.country === "United Kingdom" ? "Nation" : form.country === "United States" ? "State" : "Region"}</label><select value={form.region} onChange={e => s("region", e.target.value)} style={{ width: "100%", padding: "10px 8px", border: "1px solid var(--border)", background: "var(--card-bg)", fontSize: 13, borderRadius: 0, color: form.region ? "var(--text)" : "var(--text-muted)" }}><option value="">Select</option>{STATES_BY_COUNTRY[form.country].map(r => <option key={r} value={r}>{r}</option>)}</select></div>}
      </div>
      ...
      {form.country && PARTIES_BY_COUNTRY[form.country] && <div style={{ padding: 12, background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: 0, marginBottom: 14 }}>
        <div className="ta-field" style={{ marginBottom: 6 }}>
          ...
          <select value={form.politicalAffiliation} onChange={e => s("politicalAffiliation", e.target.value)} style={{ width: "100%", padding: "10px 8px", border: "1px solid var(--border)", background: "var(--card-bg)", fontSize: 13, borderRadius: 0, color: form.politicalAffiliation ? "var(--text)" : "var(--text-muted)" }}>
      ...
      {/* AI Agent Checkbox — near bottom so it doesn't confuse most users */}
      ...
      {/* Privacy Box */}
      <div style={{ padding: 14, background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: 0, marginBottom: 14, fontSize: 13, color: "var(--text)", lineHeight: 1.6 }}>
