...
export default function ConsensusScreen({ onViewCitizen }) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: 20, background: "var(--card-bg)", border: "1px solid #9B7DB8", borderRadius: 0, marginBottom: 20 }}>
        ...
        <p style={{ fontFamily: "var(--serif)", fontSize: 16, lineHeight: 1.5, color: "var(--text)", margin: "0 0 8px" }}>A consensus correction has survived the gauntlet. The submitter's assembly approved it. Then members of <em>other</em> assemblies independently agreed.</p>
        <p style={{ fontSize: 13, color: "var(--text)", margin: 0, lineHeight: 1.6 }}>Only the truth has the property that all people can recognize it.</p>
      </div>
      {loading ? <Loader /> : subs.length === 0 ? <Empty text="No consensus corrections yet. When a correction survives cross-group review, it appears here." /> :
        subs.map(sub => (
          <div key={sub.id} className="ta-card" style={{ borderLeft: "4px solid #7C3AED" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "var(--mono)" }}><UsernameLink username={sub.submittedBy} onClick={onViewCitizen} /> · {sub.orgName} · {fDate(sub.resolvedAt)}</span><StatusPill status="consensus" /></div>
            <a href={sub.url} target="_blank" rel="noopener" style={{ fontSize: 10, color: "var(--gold)", wordBreak: "break-all" }}>{sub.url}</a>
            <div style={{ margin: "8px 0", padding: 10, background: "var(--card-bg)", borderRadius: 0 }}>
              <SubHeadline sub={sub} />
            </div>
            <div style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.8, marginBottom: 10 }}>{sub.reasoning}</div>
