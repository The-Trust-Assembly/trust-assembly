    ...
    <div>
      ...
      <div className="ta-card" style={{ borderLeft: `4px solid ${isDIUser(u) ? "#4F46E5" : pi.color}` }}>
        ...
        {myOrgs.length > 0 && <div style={{ marginTop: 10, display: "flex", gap: 4, flexWrap: "wrap" }}>
          {myOrgs.map(o => <span key={o.id} style={{ fontSize: 10, padding: "2px 7px", fontFamily: "var(--mono)", borderRadius: 0, background: o.isGeneralPublic ? "#F0FDFA" : "var(--card-bg)", color: o.isGeneralPublic ? "#0D9488" : "#475569" }}>{o.isGeneralPublic ? "🏛" : "⬡"} {o.name}</span>)}
        </div>}
      ...
      {diAgents.length > 0 && <div className="ta-card" style={{ borderLeft: "4px solid #4F46E5" }}>
        <div style={{ fontFamily: "var(--mono)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--gold)", marginBottom: 8, fontWeight: 700 }}>🤖 Registered Digital Intelligences</div>
