...
export default function VaultScreen({ user }) {
  ...
  const OrgLabel = ({ orgId }) => { const o = orgsMap[orgId]; if (!o) return null; return <span style={{ fontSize: 9, padding: "1px 5px", fontFamily: "var(--mono)", borderRadius: 0, background: o.isGeneralPublic ? "rgba(13,148,136,0.09)" : "var(--card-bg)", color: o.isGeneralPublic ? "#0D9488" : "var(--text-sec)", marginRight: 4 }}>{o.isGeneralPublic ? "\u{1F3DB}" : "\u2B21"} {o.name}</span>; };

  const AssemblySelector = () => myOrgs.length > 1 ? (
    <div style={{ marginBottom: 10, padding: 8, background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: 0 }}>
      <div style={{ fontSize: 10, fontFamily: "var(--mono)", textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--text-sec)", marginBottom: 6 }}>Submit to assemblies: <span style={{ fontWeight: 400, textTransform: "none" }}>(select one or more)</span></div>
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        {myOrgs.map(o => { const sel = selectedOrgIds.includes(o.id); return <button key={o.id} onClick={() => toggleOrg(o.id)} style={{ padding: "4px 10px", fontSize: 10, fontFamily: "var(--mono)", border: `1.5px solid ${sel ? "#059669" : "var(--border)"}`, background: sel ? "#059669" : "var(--card-bg)", color: sel ? "#fff" : "var(--text-sec)", borderRadius: 0, cursor: "pointer", fontWeight: sel ? 700 : 400 }}>{sel ? "\u2713 " : ""}{o.isGeneralPublic ? "\u{1F3DB} " : ""}{o.name}</button>; })}
      </div>
    </div>
  ) : null;
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--text-sec)", fontSize: 12, lineHeight: 1.5, marginBottom: 14 }}>Vaults are per-assembly. Only approved entries that have survived jury review are shown. To add new entries, include them when submitting a correction through the Submit tab — all vault entries must be tied to an actual piece of media.</p>
      <div style={{ display: "flex", gap: 0, marginBottom: 16, borderBottom: "2px solid var(--border)" }}>
        {tabs.map(([k, l]) => <button key={k} onClick={() => setTab(k)} style={{ padding: "8px 14px", background: "none", border: "none", borderBottom: tab === k ? "2px solid var(--gold)" : "2px solid transparent", marginBottom: -2, fontFamily: "var(--mono)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", cursor: "pointer", color: tab === k ? "var(--gold)" : "#64748B", fontWeight: tab === k ? 700 : 400 }}>{l}</button>)}
      </div>
      {loading ? <Loader /> : <>
        {tab === "vault" && <div><p style={{ color: "var(--text-sec)", marginBottom: 14, fontSize: 13, lineHeight: 1.6 }}>Standing Corrections — reusable facts verified through jury review. Each time a correction is linked to a submission and survives review, it gains reputation.</p>{vault.length === 0 ? <Empty text="No vault entries yet. Submit one with your next correction." /> : vault.map(v => <div key={v.id} className="ta-card" style={{ borderLeft: "4px solid var(--border)" }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "var(--mono)" }}><OrgLabel orgId={v.orgId} />@{v.submittedBy} · {sDate(v.createdAt)}{v.survivalCount > 0 ? ` · survived ${v.survivalCount} review${v.survivalCount !== 1 ? "s" : ""}` : ""}</span><StatusPill status={v.status} /></div><div style={{ fontFamily: "var(--serif)", fontSize: 14, fontWeight: 600, lineHeight: 1.6 }}>{v.assertion}</div>{v.evidence && <div style={{ fontSize: 12, color: "var(--gold)", marginTop: 3 }}>{v.evidence}</div>}</div>)}</div>}
        {tab === "args" && <div><p style={{ color: "var(--text-sec)", marginBottom: 14, fontSize: 13, lineHeight: 1.6 }}>Argument Vault — store fundamental arguments your Assembly uses across corrections. Reusable rhetorical and logical tools.</p>{args.map(a => <div key={a.id} className="ta-card" style={{ borderLeft: "4px solid #0D9488" }}><div style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "var(--mono)", marginBottom: 4 }}><OrgLabel orgId={a.orgId} />@{a.submittedBy} · {sDate(a.createdAt)}{a.survivalCount > 0 ? ` · survived ${a.survivalCount} review${a.survivalCount !== 1 ? "s" : ""}` : ""}</div><div style={{ fontSize: 14, lineHeight: 1.6 }}>{a.content}</div></div>)}{args.length === 0 && <Empty text="No arguments stored yet." />}<div style={{ marginTop: 14, padding: 12, background: "var(--card-bg)", borderRadius: 0, fontSize: 12, color: "var(--text-sec)" }}>To add new arguments, include them when submitting a correction through the Submit tab. All vault entries must be tied to an actual piece of media.</div></div>}
        {tab === "beliefs" && <div><p style={{ color: "var(--text-sec)", marginBottom: 14, fontSize: 13, lineHeight: 1.6 }}>Foundational Belief Vault — core beliefs your Assembly holds as axioms. Not claims of fact but starting premises.</p>{beliefs.map(b => <div key={b.id} className="ta-card" style={{ borderLeft: "4px solid #7C3AED" }}><div style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "var(--mono)", marginBottom: 4 }}><OrgLabel orgId={b.orgId} />@{b.submittedBy} · {sDate(b.createdAt)}{b.survivalCount > 0 ? ` · survived ${b.survivalCount} review${b.survivalCount !== 1 ? "s" : ""}` : ""}</div><div style={{ fontSize: 14, lineHeight: 1.6, fontStyle: "italic" }}>{b.content}</div></div>)}{beliefs.length === 0 && <Empty text="No foundational beliefs stored yet." />}<div style={{ marginTop: 14, padding: 12, background: "var(--card-bg)", borderRadius: 0, fontSize: 12, color: "var(--text-sec)" }}>To add new foundational beliefs, include them when submitting a correction through the Submit tab. All vault entries must be tied to an actual piece of media.</div></div>}
        {tab === "trans" && <div>
          ...
          <div style={{ marginTop: 14, padding: 12, background: "var(--card-bg)", borderRadius: 0, fontSize: 12, color: "var(--text-sec)" }}>To add new translations, include them when submitting a correction through the Submit tab. All vault entries must be tied to an actual piece of media.</div>
        </div>}
      </>}
