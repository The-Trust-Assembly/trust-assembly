...
// ------------------------------------------------------
// A streamlined Sentinel-like flow available to any logged-in user.
// No persistent agent instance — just enter a thesis, generate
// keywords, run the pipeline, and review results.
//
// Differences from SentinelDashboard:
//   - No agent_instance_id on the run
...
export default function OneTimeDashboard({ onReview }) {
  ...
  async function handleGenerateKeywords() {
    if (!thesis.trim()) { setError("Please enter a thesis first."); return; }
    setError("");
  ...
  function removeKeyword(i) { setKeywords(keywords.filter((_, idx) => idx !== i)); }

  function addKeyword() {
    const k = newKeyword.trim();
    if (k && !keywords.includes(k)) { setKeywords([...keywords, k]); setNewKeyword(""); }
  }
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 24px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: 13, margin: 0 }}>
          Enter what you think needs checking. The AI will search the web, analyze articles, and
          draft corrections or affirmations for your review.
        </p>
      </div>
      ...
      >
        ...
        <textarea
          ...
          placeholder="e.g., Several news outlets are reporting that the bill was passed unanimously, but the actual vote was 52-48."
          style={{
        ...
        {showKeywords && (
          <>
            <div style={{ marginTop: 16 }}>
              ...
              >
                ...
                <span style={{ fontWeight: 400, color: "var(--text-muted)" }}>
                  ({keywords.length} · click x to remove)
                </span>
              </label>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                ...
                />
                <button
                  type="button" className="ta-btn-secondary"
                  onClick={addKeyword} disabled={!newKeyword.trim()}
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
              <button
                type="button" className="ta-btn-secondary"
                onClick={() => { setShowKeywords(false); setKeywords([]); }}
                disabled={running} style={{ fontSize: 13 }}
              >
                ← Edit Thesis
              </button>
              <button
              ...
              >
                {running ? "Starting..." : `Search with ${keywords.length} Keyword${keywords.length === 1 ? "" : "s"}`}
              </button>
            </div>
          </>
        )}
